...
class Game:
   ...
   def is_win(self):
      win = False
      # If any of these are True return True so the game is turned off 
      row_win = self.is_row_win()
      #col_win = self.is_col_win()
      #diagonal_win = self.is_diagonal_win()

      if row_win:
         #or col_win or diagonal_win :
         win = True
      return win
   
   
   def is_row_win(self):
      # collect all the rows in board and put these inside a list 
      row_win = False
      # list_of_lists_of_tiles -----> [[T0,T1,12],[T3,T4,15],[T6,T7,18]]
   ...
   def is_list_win(self, list_of_tiles): # [T0,T1,12]
      pass   
...
class Tile:
   ...
   def is_equal(self, other):
      ...
      if self.hidden == False and other.hidden == False and self.content == other.content:
         self.hidden = False
         other.hidden = False
         return True 
